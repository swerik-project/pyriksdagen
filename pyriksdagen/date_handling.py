    ...
    if start_year == end_year or end_year == None:
        ...
        # if an year is an acceptable parliament year
            ...
            # else
                ...
                try:
                    df = riksmote.filter(pl.col("parliament_year") == int(start_year + f"{int(str(start_year)[2:])+1:02}"))
                    #print(df)
                    ...
                    if start_p == "day":
                        df_starts = df["start"].unique().to_list()
                ...
                try:
                    ...
                    df = riksmote.filter(pl.col("parliament_year") == xx)
                    assert df.is_empty() == False
                except:
                ...
                for ix, y in enumerate(y_range):
                    ...
                    if str(y) in year_list and str(y) != "1975":
                        ...
                        for a in to_append:
                            if ix == 0:
                                try:
                                    df = riksmote.filter(pl.col("parliament_year") == int(a))
                                    assert df.is_empty() == False
                                except:
...
def yearize_date(date, riksmote):
    ...
    """
    parliament_years = riksmote["parliament_year"].unique().to_list()
...
def yearize_mandates(debug_id=None, metadata_folder="data"):
    ...
    riksmote = pl.read_csv(f"{metadata_folder}/riksdag-year.csv")
    mep = pl.read_csv(f"{metadata_folder}/member_of_parliament.csv")
    minister = pl.read_csv(f"{metadata_folder}/minister.csv")
    speaker = pl.read_csv(f"{metadata_folder}/speaker.csv")
    government = pl.read_csv(f"{metadata_folder}/government.csv")
    parliament_years = riksmote["parliament_year"].unique().to_list()
    parliament_years = [str(_) for _ in parliament_years]
    rows = []
    slen = []
    ...
    for k, df in {"member_of_parliament":mep, "minister": minister, "speaker": speaker}.items():
        ...
        for r in tqdm(df.iter_rows(named=True), total=len(df)):
            ...
            if start_y and 1866 < int(start_y[:4]) < 2025:
                ...
                for ix, py in enumerate(pys):
                    if int(py) < 202324:
                        try:
                            py_start = riksmote.filter(pl.col("parliament_year") == int(py))["start"].unique()[0]
                        except:
                            print("start")
                            print(py, type(py), pys, start, end, start_y, end_y)
                        try:
                            py_end = riksmote.filter(pl.col("parliament_year") == int(py))["end"].unique()[-1]
                        except:
                            print("end")
